...
def suggest_keywords_from_jd(text, top_n=15):
    ...
    STOP_WORDS = [
        'experience', 'Developer','team', 'work', 'job', 'role', 'company', 'candidate', 'skill',
        'skills', 'requirements', 'responsibilities', 'knowledge', 'ability',
        'client', 'project', 'projects', 'business', 'solution', 'solutions', 'system',
        'systems', 'data', 'product', 'products', 'development', 'management', 'analysis',
        'design', 'report', 'reports', 'service', 'services', 'application', 'applications', 'inc'
    ]
    ...
    most_common_keywords = [word for word, count in Counter(keywords).most_common(top_n)]
...
def generate_score_explanation(scores, candidate_months, required_months):
    ...
    if relevance_score > 0.7: relevance_reason = "Excellent semantic match..."
    elif relevance_score > 0.55: relevance_reason = "Good semantic match..."
    else: relevance_reason = "Low semantic match..."
    if keyword_score == 1.0: keyword_reason = "Excellent keyword match..."
    else:
    ...
    if experience_score >= 1.0: experience_reason = f"Excellent experience. Meets or exceeds the {required_years:.1f}-year requirement."
    else: experience_reason = f"Low experience. Less than the required {required_years:.1f} years."
...
def score_and_rank(job_description, required_years, keywords, model, db_df, user_resume_info=None):
    ...
    db_df['keyword_score'] = db_df['resume_text'].apply(lambda x: calculate_keyword_score(x, keywords)[0])
    ...
    if user_resume_info:
        ...
        user_keywords, missing_kws = calculate_keyword_score(user_resume_info['text'], keywords)
        user_experience = min((user_resume_info['years_exp'] * 12) / target_months, 1.0) if target_months > 0 else 1.0
    ...
    try:
        ...
        if isinstance(row.get('skills'), dict) and 'technical' in row['skills']:
            for skill_cat in row['skills']['technical'].values():
                if isinstance(skill_cat, list): skills_list.extend([s.get('name', '') for s in skill_cat])
        responsibilities = []
        if isinstance(row.get('experience'), list):
            for job in row['experience']:
                if isinstance(job.get('responsibilities'), list): responsibilities.extend(job['responsibilities'])
...
def main():
    ...
    print("➡️ Please review the list above and update the `MUST_HAVE_KEYWORDS` list below if needed.\n")
    ...
    if os.path.exists(PROCESSED_DATA_PATH):
        ...
        try:
            ...
            print("⚠️ 'resume.jsonl' not found. Ranking against other candidates will be skipped.")
            resumes_df = pd.DataFrame()
        
        if not resumes_df.empty:
            resumes_df['total_months_experience'] = resumes_df['experience'].apply(
                lambda x: sum(int(re.search(r'(\d+)', job.get('dates', {}).get('duration', '0')).group(1)) for job in x if isinstance(x, list))
            )
            ...
            resumes_df['resume_vector'] = resumes_df['resume_text'].apply(lambda x: model.encode(x))
    ...
    if user_resume_text:
        ...
        ranked_candidates, user_scores = score_and_rank(
            JOB_DESCRIPTION, REQUIRED_EXPERIENCE_YEARS, MUST_HAVE_KEYWORDS, model, resumes_df, user_info
        )

        user_score_reason = generate_score_explanation(
            user_scores,
            CANDIDATE_EXPERIENCE_YEARS * 12,
